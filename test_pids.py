...
rollrate_pid = PID_controller(rollrate_kp, rollrate_ki, rollrate_kd, rollrate_kff, 1 / freq_attitude_rate, freq_attitude_rate, omxFiltCut, True)
pitchrate_pid = PID_controller(pitchrate_kp, pitchrate_ki, pitchrate_kd, pitchrate_kff, 1 / freq_attitude_rate, freq_attitude_rate, omyFiltCut, True)
yawrate_pid = PID_controller(yawrate_kp, yawrate_ki, yawrate_kd, yawrate_kff, 1 / freq_attitude_rate, freq_attitude_rate, omzFiltCut, True)
...
def rate_controller(onboard, i):
    gx, gy, gz = onboard.loc[i, ["gyro.x", "gyro.y", "gyro.z"]]
    pitchrate_sp, rollrate_sp, yawrate_sp = onboard.loc[
        i, ["controller.rollRate", "controller.pitchRate", "controller.yawRate"]
    ]  # .pitchRate, .rollRate, .yawRate is the setpoint according to craziflie docs, check controller_pid.c

    pitchrate_sp = np.array(pitchrate_sp) - onboard.loc[0, "controller.pitchRate"]

    controller_rollrate.append(rollrate_pid.compute(gx, rollrate_sp, False))
    controller_pitchrate.append(pitchrate_pid.compute(gy, pitchrate_sp, False))
    controller_yawrate.append(yawrate_pid.compute(gz, yawrate_sp, False))
...
if __name__ == "__main__":
    ...
    for i in range(len(onboard_data)):
        rate_controller(onboard_data, i)
    ...
    plt.plot(np.array(controller_pitchrate) - controller_pitchrate[0])
    plt.plot(onboard_data["controller.cmd_pitch"], alpha=0.5)
